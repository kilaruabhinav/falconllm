...
class CalculatorTool(BaseTool):
    ...
    def execute(self, arguments: Mapping[str, Any]) -> ToolResult:
        ...
        if not isinstance(expression, str):
            return ToolResult(success=False, tool=self.name, error="Expression must be a string.")
        expression = expression.strip()
        if not expression:
            return ToolResult(success=False, tool=self.name, error="Expression cannot be empty.")
        if len(expression) > self._MAX_EXPRESSION_LENGTH:
            return ToolResult(success=False, tool=self.name, error="Expression is too long.")
        ...
        try:
            ...
            return ToolResult(success=True, tool=self.name, result=result)
        except ZeroDivisionError:
            return ToolResult(success=False, tool=self.name, error="Cannot divide by zero.")
        except (SyntaxError, TypeError, ValueError, OverflowError):
            return ToolResult(success=False, tool=self.name, error="Invalid mathematical expression.")
